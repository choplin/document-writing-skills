...
Judge whether the intended reader has the conceptual understanding
...
Judge whether one concept is named by one term throughout and whether each term
keeps one status across the document.
...
- **Orthographic variants** of one term (spacing, kana/kanji, hyphenation,
  capitalization) usually create a local consistency defect.

### Reader impact

One term denoting two indistinguishable concepts can prevent understanding.
Aliasing and retreat to a vague word have broader impact than an isolated
...
Find expressions whose referent the reader cannot resolve, or can
...
The defect can prevent understanding when no referent exists or two are equally
available. A referent recoverable only by re-reading still interrupts the local
flow.
...
Judge whether each focus, contrast, negation, metaphor, and retrospective
reference relies only on a question, alternative, proposition, or mapping
available to the reader at that point.
...
The impact is substantial where a passage answers a question, rejects an
alternative, or spends a metaphor that the reader has had no reason to
construct. It is more local where the required state is available only after
avoidable re-reading.
